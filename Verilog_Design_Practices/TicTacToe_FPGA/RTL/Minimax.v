module Minimax (
    ...
    (* keep = "true" *) reg [3:0]           loop_pos;

    (* keep = "true" *) reg [3:0]           current_level;
    (* keep = "true" *) reg [17:0]          current_board;
    (* keep = "true" *) reg                 current_is_max;
    (* keep = "true" *) reg signed [7:0]    current_best;
    (* keep = "true" *) reg [3:0]           current_try_pos;
    ...
    always @(*) begin
        case (current_state)
            ...
            SCAN: begin
                if (root_pos == 4'd9)
                    next_state = DONE;
                else if (cell_at(test_board, root_pos) == EMPTY)
                    next_state = WAIT;
                else
                    next_state = SCAN;
            end
    ...
    always @(posedge CLK or negedge RST) begin
        if (RST == 0) begin
            ...
            if (sp > 0) begin
                ...
                else if (win_x(current_board)) begin
                    node_score      = 8'sd99 - sp;
                    node_backtrack  = 1;
                end 
                else if (board_full(current_board)) begin
                    node_score      = 8'sd0;
                    node_backtrack  = 1;
                end 
                else begin
        ...
        if (RST == 0) begin
            ...
            case (current_state)
                ...
                SCAN: begin
                    ...
                    else if (cell_at(test_board, root_pos) == EMPTY) begin
